...
test("2 · La función registrar no devuelve valor (void) y agrega al historial", () => {
...
test("3 · La función fallar nunca retorna (never) y lanza el error", () => {
  expectTypeOf(fallar).returns.toEqualTypeOf<never>();
  expect(() => fallar("boom")).toThrow("boom");
});

test("4 · La función nuncaTermina nunca retorna algo (never)", () => {
  expectTypeOf(nuncaTermina).returns.toEqualTypeOf<never>();
});

test("5 · La función inspeccionar recibe unknown y no devuelve nada (void)", () => {
  expectTypeOf(inspeccionar).parameter(0).toEqualTypeOf<unknown>();
  expectTypeOf(inspeccionar).returns.toEqualTypeOf<void>();
});

test("6 · La función registrarVarias agrega el mensaje varias veces", () => {
